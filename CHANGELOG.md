...
### Changed
- **Drain-then-reconcile ordering is now enforced entirely by `applyDrainedEventsToState`** running before reconcile. `LocationTracker.drainPendingEvents` calls `applyDrainedEventsToState` on the drained batch so `zoneStates` reflects the post-drain truth; the subsequent `reconcileZoneStates` then does its normal mismatch check and fires `RECOVERY_ENTER/RECOVERY_EXIT` only where a genuine mismatch remains (e.g. the eviction/re-entry case where a later crossing was dropped from the queue so the drain's final state disagrees with GPS). `reconcileZoneStates` keeps its 1-arg signature — no new parameter, no ABI break, no bytecode-level compatibility work required.
- **`GeofenceEngine` gains a `reconcileLock`** (a plain `Object` monitor on Android, an `NSLock` on iOS) held across both `reconcileZoneStates` and `applyDrainedEventsToState`. Serialises the two operations so a reconcile that fires from a location callback thread while a drain is mid-apply cannot see half-updated `zoneStates` and produce a spurious `RECOVERY_ENTER/EXIT`.
