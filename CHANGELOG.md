...
- **OS-managed wake fences — `osGeofenceWakeEnabled` on `PolyfenceConfig` (default `false`)** — an opt-in wake source that lets a crossing be captured after the polyfence-core process is fully killed (OOM, force-stop, task-manager swipe). Polyfence's own containment engine is unchanged and remains the sole detector: unlimited zones, full polygon support, all math in-process. When enabled, the library additionally mirrors the **top-N-nearest** active zones to the platform geofence API (`GeofencingClient` on Android, `CLCircularRegion` region monitoring on iOS) purely as an alarm clock. An OS-fired transition is written straight into the same `PendingEventsStore` the durable queue uses and delivered through the same drain path on the next tracker boot — no new event channel, no new callback, no new event type. Polygon zones are registered as a circular bounding cover around their centroid; the polygon's real containment math still runs in-engine on drain, so the cover only ever over-triggers the wake, never changes a verdict. Requires `pendingEventsQueueSize > 0` to be useful — the wake has nowhere to deposit the crossing otherwise. **Caps:** iOS monitors at most **20** regions per app (Apple's limit) and Android at most **90**; beyond that the nearest N to the last known fix are covered and the rest rely on the normal polling engine. The registered set is recomputed when the zone set changes (coalesced over a 200 ms window so a bulk `addZone` loop produces one registration) and when the device moves more than 1 km from the fix that seeded the current selection. **Known limitation:** Android's platform geofences do not survive a device reboot and are re-registered on the next `startTracking()`, so a crossing during a reboot-to-first-launch window is not captured.
...
- **Android** — the runtime `ACCESS_BACKGROUND_LOCATION` grant. The library declares the permission in its manifest (it was already an effective requirement: `LocationTracker` refuses to start tracking on API 29+ without it), but the runtime prompt and its rationale UI belong to the consumer app. Google Play requires a background-location declaration for apps that request it.
